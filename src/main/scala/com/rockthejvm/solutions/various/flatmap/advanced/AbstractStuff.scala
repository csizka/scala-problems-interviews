...
object AbstractStuff {
  ...
  // HINT:                                                                                                                                                                                                                try using recursion / foldLeft       +       you'll also need to match on the input vector, and flatMap on the first element         + if running into Option vs Some issues, use the Option constructor instead of Some
  ...
  // HINT:                                                                                                                                                                                                                try using recursion / foldLeft       +       you'll also need to match on the input vector, and flatMap on the first element         + if running into Either vs Right issues, add a type annotation to the value constructed with Right liek so 'Right(5) : Either[String, Int]'
  ...
  // in fact, this the implementation is the same for anything tha supports flatMap + map + pure
...
object AbstractStuffTests extends App {
  ...
  // NOTE: usually the compputation in the tuple are not so simple, for example:
  assert(
    tupledOption(
      getFilms("John", 2000),
      getFilms("Terry", 2003),
